...
const BcsQuotation = () => {
  ...
  const generatePDF = (count) => {
   ...
   let cell1 = formData.no1Rate;
    ...
    pdf.text(`${cell1}`, 123, 78,'right');
    pdf.text(`${formData.qty1}`, 146, 78,'right');
    let amount1 = formData.no1Rate * formData.qty1;
    if (isNaN(amount1) || amount1 === 0) {
      amount1 = "";
    }
    if(formData.qty1 === "" || formData.qty1 === undefined ){
      formData.qty1 = 1
      amount1 = formData.no1Rate 
      cell1 = " "
    }
    ...
    pdf.text(`${subTotal}`, 185, 185,"right");
    ...
    pdf.text(`${gstAmountTotal}`, 185, 195,'right');
  
    pdf.text(` Grand Total`, 115, 205);
    const grandTotal = (parseFloat(gstAmountTotal) || 0) + parseFloat(subTotal);


    if (isNaN(grandTotal) || grandTotal === 0) {
      grandTotal = "";
    }
    pdf.text(`${grandTotal}`, 185, 205,'right');
    pdf.setFontSize(12);
    ...
    const pdfName = `Crane_Quotation${sanitizedName}.pdf`;
